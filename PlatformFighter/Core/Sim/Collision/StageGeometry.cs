...
/// parallel <c>startingPoint</c>/<c>startingFace</c> (and <c>respawnPoints</c>/<c>respawnFace</c>)
/// arrays, collapsed into one struct per point instead of
...
/// CameraBounds/Ledges/StartingPoints/RespawnPoints) hold real per-stage
/// data transcribed from MeleeLight (see Stages/Battlefield.cs), additive
/// to the Solids/Platforms model above rather than replacing it —
/// TestStage.cs's simple flat-floor stage (used by DeterminismTest etc.)
/// still only populates Solids/Platforms and leaves these new lists empty,
/// and nothing currently reads them (see the class-level scope note on why:
/// the real ECB collision RESOLUTION algorithm these segments need is
/// Step 2's job, not this one).
/// </summary>
public sealed class StageGeometry
{
	...
	/// <summary>Per-stage camera limit box — the region the framing camera is
	/// allowed to look at, in sim units. Real Melee drives its camera from a
	/// per-stage limit box (cam_limit_top/right/bottom/left in the decompiled
	/// camera code, see walz0/MeleeCameraUE5) that clamps the subject-derived
	/// framing bounds; the camera never shows beyond it even when a fighter is
	/// knocked past it toward the blast zone. MeleeLight has NO camera data of
	/// its own (confirmed in Stages/Battlefield.cs's scope notes), so unlike
	/// Ground/BlastZone/etc. this is NOT a MeleeLight port — it is real-Melee
	/// camera structure with per-stage values authored in each stage file.
	/// Null if the stage defines no camera box, in which case CameraController
	/// falls back to framing on the blast zone (or an unclamped subject bounds
	/// if neither exists) rather than throwing.</summary>
	public FxAabb? CameraBounds;
	...
	public StageGeometry SetCameraBounds(FxAabb cameraBounds) { CameraBounds = cameraBounds; return this; }
	...
	public bool IsPastBlastZone(FxVec2 position)
		=> BlastZone.HasValue && !BlastZone.Value.Contains(position);
}
